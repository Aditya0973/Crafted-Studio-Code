...
import { MonacoEditorHost } from '../workbench/MonacoEditorHost';
import { WorkbenchErrorBoundary } from '../workbench/WorkbenchErrorBoundary';
...
export const WorkbenchArea: React.FC = () => {
  ...
  // VS Code Behavior: Tab Strip Auto-Scroll to Active Tab (Part 5)
  useEffect(() => {
    ...
    >
      ...
      {openTabs.length > 0 ? (
        <div className="flex h-9 items-center justify-between border-b border-crafted-border/60 bg-[#181818] px-1 shrink-0">
          {/* Scrollable Tab Strip with Auto-Scroll Support */}
          <div className="flex items-center space-x-0.5 overflow-x-auto h-full min-w-0 pr-2 scrollbar-none">
            {openTabs.map((tab: TabItem) => {
              ...
              return (
                ...
                >
                  <FileCode className={`h-3.5 w-3.5 shrink-0 ${isActive ? 'text-cyan-400' : 'text-crafted-text-dim'}`} />
      ...
      {/* Main Viewport Content wrapped in React Error Boundary */}
          ...
          {activeTab ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-6 text-center text-crafted-text-dim space-y-3 font-sans">
              ...
              <p className="text-xs max-w-sm text-crafted-text-muted">
                Select or double-click a file from the Workspace Explorer to open it in Monaco Editor.
              </p>
            </div>
          )}
        </WorkbenchErrorBoundary>
